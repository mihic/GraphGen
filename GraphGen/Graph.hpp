...
Graph RandomGraph(int n, std::default_random_engine random_engine, double density, int cmin, int cmax) {
  Graph G(n);
...
Graph Random2DGridGraph(int n, std::default_random_engine random_engine, double density, int cmin, int cmax) {
  std::uniform_int_distribution<int> r_int(cmin, cmax);
  std::uniform_real_distribution<double> r_double(0, 1);
  int new_n = int(std::sqrt(n));
  int n2 = new_n * new_n;
    if (n2 != n) {
